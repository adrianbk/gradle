...
class MavenPublishS3SnapshotIntegrationTest extends AbstractIntegrationSpec {
    ...
    boolean assertSnapshotPublished(List<File> files, int num = 1) {
        ...
        def mainArtifact = files.find { it.absolutePath ==~ /${baseExpr}${snapshotArtifactExpr}-${num}\.jar/ }
        assert mainArtifact
        assert files.find { it.absolutePath ==~ /${baseExpr}${snapshotArtifactExpr}-${num}\.jar.sha1/ }
        assert files.find { it.absolutePath ==~ /${baseExpr}${snapshotArtifactExpr}-${num}\.jar.md5/ }

        assert files.find { it.absolutePath ==~ /${baseExpr}${snapshotArtifactExpr}-${num}\.pom/ }
        assert files.find { it.absolutePath ==~ /${baseExpr}${snapshotArtifactExpr}-${num}\.pom.sha1/ }
        assert files.find { it.absolutePath ==~ /${baseExpr}${snapshotArtifactExpr}-${num}\.pom.md5/ }
